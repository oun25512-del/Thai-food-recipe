...
// ===== COMMENTS =====
function commentKey(name) { return 'cmt_' + name.replace(/\s/g,'_'); }

function loadComments(name) {
  try { return JSON.parse(localStorage.getItem(commentKey(name)) || '[]'); } catch(e) { return []; }
}
function saveComments(name, list) {
  try { localStorage.setItem(commentKey(name), JSON.stringify(list)); } catch(e) {}
}
...
function renderComments(name) {
  const list = loadComments(name);
  const box = document.getElementById('cmt-list');
  const countEl = document.getElementById('cmt-count');
  if (countEl) countEl.textContent = list.length + ' ความคิดเห็น';
  if (!box) return;
  if (!list.length) {
    box.innerHTML = '<p class="no-comment">💬 ยังไม่มีความคิดเห็น เป็นคนแรกที่แสดงความคิดเห็น!</p>';
    return;
  }
  box.innerHTML = list.slice().reverse().map((c, ri) => {
    const realIdx = list.length - 1 - ri;
    return `<div class="comment-card">
      <button class="comment-del" data-idx="${realIdx}" title="ลบ">✕</button>
  ...
  box.querySelectorAll('.comment-del').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      const updated = loadComments(name);
      updated.splice(idx, 1);
      saveComments(name, updated);
      renderComments(name);
    });
  });
}

function buildCommentSection(name) {
  const list = loadComments(name);
  return `<div class="comment-section">
    <div class="comment-title">💬 ความคิดเห็น <span class="comment-count" id="cmt-count">${list.length} ความคิดเห็น</span></div>
    <div class="comment-form">
      ...
      <div class="comment-inputs">
        <input id="cmt-name" type="text" placeholder="ชื่อของคุณ" maxlength="30" />
      </div>
      <textarea id="cmt-text" class="comment-textarea" placeholder="แสดงความคิดเห็น เช่น รสชาติเป็นอย่างไร ทำยากไหม..." maxlength="300"></textarea>
      <button class="btn-post" id="btn-post-cmt">📨 โพสต์ความคิดเห็น</button>
    </div>
    <div class="comment-list" id="cmt-list"></div>
  </div>`;
}

function initCommentSection(name) {
  renderComments(name);
  ...
  function updateStars(val) {
    stars.forEach(s => {
      s.classList.toggle('on', parseInt(s.getAttribute('data-v')) <= val);
    });
  }
  updateStars(selectedRating);

  stars.forEach(s => {
    s.addEventListener('mouseenter', () => updateStars(parseInt(s.getAttribute('data-v'))));
    s.addEventListener('mouseleave', () => updateStars(selectedRating));
    s.addEventListener('click', () => { selectedRating = parseInt(s.getAttribute('data-v')); updateStars(selectedRating); });
  });

  document.getElementById('btn-post-cmt').addEventListener('click', function() {
    const nameVal = document.getElementById('cmt-name').value.trim();
    const textVal = document.getElementById('cmt-text').value.trim();
    if (!nameVal) { document.getElementById('cmt-name').style.borderColor = 'var(--accent)'; document.getElementById('cmt-name').focus(); return; }
    if (!textVal) { document.getElementById('cmt-text').style.borderColor = 'var(--accent)'; document.getElementById('cmt-text').focus(); return; }
    const list = loadComments(name);
    list.push({ name: nameVal, text: textVal, rating: selectedRating, ts: Date.now() });
    saveComments(name, list);
...
function openDetail(r) {
  ...
  const steps = r.steps.map((s, i) => {
    const mins = r.stepMins && r.stepMins[i] ? `<span style="font-size:10px;background:var(--tag-bg);color:var(--tag-text);padding:1px 7px;border-radius:10px;margin-left:6px;font-weight:600;flex-shrink:0">⏱ ${r.stepMins[i]} นาที</span>` : '';
    return `<li><div class="step-num">${i + 1}</div><span>${s}${mins}</span></li>`;
  }).join('');
